"use strict";if('serviceWorker'in navigator){window.addEventListener('load',()=>{navigator.serviceWorker.register('service-worker.js').then((reg)=>{console.log('Service worker registered.',reg);});});}
if(jQuery('[data-fancybox="gallery"]').length>0){$('[data-fancybox="gallery"]').fancybox({});}
...
var education_details=$('.education-experience form').html();$('.education-experience span.add-btn').on('click',function(){$(".education-experience form").append(education_details);f1();});var work_details=$('.work-experience form').html();$('.work-experience span.add-btn').on('click',function(){$(".work-experience form").append(work_details);f1();});var awards_details=$('.awards-experience form').html();$('.awards-experience span.add-btn').on('click',function(){$(".awards-experience form").append(awards_details);f1();});var membership_details=$('.membership-experience form').html();$('.membership-experience span.add-btn').on('click',function(){$(".membership-experience form").append(membership_details);f1();});var registration_details=$('.registration-col form').html();$('.registration-col span.add-btn').on('click',function(){$(".registration-col form").append(registration_details);f1();});function getLocation(){if(navigator.geolocation){navigator.geolocation.getCurrentPosition(showPosition);}else{console.log("Geo Location not supported by browser");}}
function showPosition(position){var location={longitude:position.coords.longitude,latitude:position.coords.latitude}
console.log(location)}
getLocation();function locationError(error){switch(error.code){case error.PERMISSION_DENIED:return "User denied the request for Geolocation."
break;case error.POSITION_UNAVAILABLE:return "Location information is unavailable."
break;case error.TIMEOUT:return "The request to get user location timed out."
break;case error.UNKNOWN_ERROR:return "An unknown error occurred."
break;}}
function showInMap(pos){var latlon=pos.coords.latitude+","+pos.coords.longitude;var img_url="https://maps.googleapis.com/maps/api/staticmap?center="+latlon+"&zoom=14&size=400x300&sensor=false&key=YOUR_:KEY";var map=document.querySelector("mapholder");map.innerHTML="<img src='"+img_url+"'>";}
$(".add-hours").on('click',function(){var hourscontent='<div class="row form-row hours-cont">'+
